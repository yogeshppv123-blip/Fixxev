...
class FranchiseEditScreen extends StatefulWidget {
  final Map<String, dynamic>? franchiseType;
  const FranchiseEditScreen({super.key, this.franchiseType});
...
class _FranchiseEditScreenState extends State<FranchiseEditScreen> {
  ...
  final List<String> _types = ['Spare Parts', 'Service Center'];

  @override
  void initState() {
    super.initState();
    if (widget.franchiseType != null) {
      _titleController.text = widget.franchiseType!['title'] ?? '';
      _descriptionController.text = widget.franchiseType!['description'] ?? '';
      _imageUrlController.text = widget.franchiseType!['imageUrl'] ?? '';
      _selectedType = widget.franchiseType!['type'] ?? 'Spare Parts';
      _isActive = widget.franchiseType!['isActive'] ?? true;
      if (widget.franchiseType!['benefits'] != null) {
        _benefitsController.text = (widget.franchiseType!['benefits'] as List).join(', ');
      }
    }
  }
  ...
  Future<void> _saveFranchiseType() async {
    ...
    try {
      ...
      if (widget.franchiseType != null) {
        await _apiService.updateFranchiseType(widget.franchiseType!['_id'], data);
      } else {
        await _apiService.createFranchiseType(data);
      }

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(widget.franchiseType != null ? 'Franchise type updated!' : 'Franchise type created!')),
        );
  ...
  Widget build(BuildContext context) {
    final isEditing = widget.franchiseType != null;
                ...
                Expanded(
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      children: [
                        _buildSectionCard(
                          title: 'Franchise Type Details',
                          icon: Icons.handshake_outlined,
                          children: [
                            _buildTextField('Title', _titleController, 'e.g., Spare Parts Dealer'),
                            const SizedBox(height: 16),
                            _buildDropdown(),
                            const SizedBox(height: 16),
                            _buildTextField('Description', _descriptionController, 'Describe this franchise type...', maxLines: 4),
                            const SizedBox(height: 16),
                            _buildTextField('Benefits (Comma separated)', _benefitsController, 'Benefit 1, Benefit 2, Benefit 3', maxLines: 3),
                            const SizedBox(height: 16),
                            _buildImageField(),
                            const SizedBox(height: 16),
                            _buildActiveSwitch(),
        ...
        children: [
          Row(
            children: [
              ...
              Text(isEditing ? 'Edit Franchise Type' : 'Add Franchise Type', style: AppTextStyles.heading2),
            ],
          ),
          ElevatedButton.icon(
            onPressed: _isSaving ? null : _saveFranchiseType,
            icon: _isSaving
